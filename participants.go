...
func (participants *Participants) GetJoinURL() string {
	...
	// 构造必填参数
	fullName := "fullName=" + participants.FullName_     // 用户名
	meetingID := "&meetingID=" + participants.MeetingID_ // 试图加入的会议ID
	password := "&password=" + participants.Password_    // 会议室密码, 这里特指与会者密码, 如果传入管理员密码则以管理员身份进入
	...
	if "" != participants.CreateTime {
		createTime = "&createTime=" + participants.CreateTime
	}

	if "" != participants.UserID {
		userID = "&userID=" + participants.UserID
	}

	if "" != participants.ConfigToken {
		configToken = "&configToken=" + participants.ConfigToken
	}

	if "" != participants.AvatarURL {
		avatarURL = "&avatarURL=" + participants.AvatarURL
	}

	if "" != participants.ClientURL {
		redirect = "&redirect=true"
		clientURL = "&clientURL=" + participants.ClientURL
	}
